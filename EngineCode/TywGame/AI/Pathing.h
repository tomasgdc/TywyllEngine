#ifndef _AI_PATHING_H_
#define _AI_PATHING_H_
...
{
	float m_tolerance;
	glx::vec3<float> m_pos;
	PathingArcList m_arcs;

public:
	explicit PathingNode(const glx::vec3<float>& pos, float tolerance = PATHING_DEFAULT_NODE_TOLERANCE) : m_pos(pos) { m_tolerance = tolerance; }
	const glx::vec3<float>& GetPos(void) const { return m_pos; }
...
public:
	...
	const glx::vec3<float>& GetCurrentNodePosition(void) const { assert(m_index != m_path.end()); return (*m_index)->GetPos(); }
	bool CheckForNextNode(const glx::vec3<float>& pos);
...
public:
	...
	PathingNode* FindClosestNode(const glx::vec3<float>& pos);
	PathingNode* FindFurthestNode(const glx::vec3<float>& pos);
	PathingNode* FindRandomNode(void);
	PathPlan* FindPath(const glx::vec3<float>& startPoint, const glx::vec3<float>& endPoint);
	PathPlan* FindPath(const glx::vec3<float>& startPoint, PathingNode* pGoalNode);
	PathPlan* FindPath(PathingNode* pStartNode, const glx::vec3<float>& endPoint);
