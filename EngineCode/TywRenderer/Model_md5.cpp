...
void MD5_ParseTriangles(FILE* ptrFile, std::vector<GLuint> &tri, uint32_t numsTriangles) {
...
void MD5Mesh::ParseMesh(FILE* ptrFile, int numJoints, std::vector<JointMat>& joints) 
{
	...
	while(rval != 0 && rval != EOF)
	{
		...
		if (strcmp(lineHeader, "shader") == 0) 
		{
			...
			shader->setTexture(globalImage->GetImage(shaderName), false);
		}
	...
	for (auto& i : verts)
	{
		...
		glx::vec3<float> v(0.0f, 0.0f, 0.0f);
		drawVert		 pose;
		for (int j = 0; j < i.numWeightsForVertex; j++)
		{
			...
			glx::vec3<float> rotPos = joint * tempWeight.pos;
...
bool RenderModelMD5::ParseJoint(FILE* ptrFile, MD5Joint& joint, JointQuat& pose) {
	...
	char			junk[256];
		...
		if (!ferror) 
		{
			...
			//breaks	
			puts(strLine);
			int32_t rval = sscanf(strLine, "%s %i %s %f %f %f %s %s %f %f %f %s", joint.name, &joint.parentID, junk, &pose.t.x, &pose.t.y, &pose.t.z, junk, junk, &pose.q.x, &pose.q.y, &pose.q.z);
			if (rval != 12)
			{
				return false;
			}
		...
		if (strcmp(lineHeader, "MD5Version") == 0) {
			...
			for (int i = 0; i < numJoints; i++)
			{
				...
				if (ParseJoint(file, joint, pose))
				{
					...
					pMat.SetRotation(pose.q.ToMat4());
...
void MD5Mesh::UpdateMesh(const MD5Mesh *mesh,  std::vector<JointQuat>& joints, const	JointMat *entJointsInverted, deformInfo_t *surf)
{
	...
	for (auto& i : verts)
	{
		...
		for (int j = 0; j < i.numWeightsForVertex; j++)
		{
			...
			JointMat mat;
			mat.SetRotation(joint.q.ToMat4());
			mat.SetTranslation(joint.t);

			// Convert the weight position from Joint local space to object space
			glx::vec3<float> rotPos = mat * tempWeight.pos;
