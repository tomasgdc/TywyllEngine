...
bool XmlResourceLoader::VLoadResource(char *rawBuffer, unsigned int rawSize, std::shared_ptr<ResHandle> handle)
{
    ...
    std::shared_ptr<XmlResourceExtraData> pExtraData = std::shared_ptr<XmlResourceExtraData>(TYW_NEW XmlResourceExtraData());
...
{
    return std::shared_ptr<IResourceLoader>(TYW_NEW XmlResourceLoader());
}
