...
#include "math\GLXVec3.h"
#include "Utilities/EngineString.h"
...
{ 
	...
	if ( m_file->VOpen() )
	{
		RegisterLoader(std::shared_ptr<IResourceLoader>(TYW_NEW DefaultResourceLoader()));
		retValue = true;
	}
	return retValue;
}
...
std::shared_ptr<ResHandle> ResCache::Load(Resource *r)
{
	...
	char *rawBuffer = loader->VUseRawFile() ? Allocate(allocSize) : TYW_NEW char[allocSize];
	...
	{
		buffer = rawBuffer;
		handle = std::shared_ptr<ResHandle>(TYW_NEW ResHandle(*r, buffer, rawSize, this));
	}
	else
	{
		...
		handle = std::shared_ptr<ResHandle>(TYW_NEW ResHandle(*r, buffer, size, this));
